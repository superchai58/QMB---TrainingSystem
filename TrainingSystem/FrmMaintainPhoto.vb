...
Public Class FrmMaintainPhoto
    ...
    Private Sub FrmMaintainPhoto_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If TStation.ToUpper() = "TRAINING" Then
            ...
            StrSql = "Select distinct [group] from Exercise_Detail As A With(nolock) inner join OP_ExamSubject As B With(nolock) On A.[Group] = B.Title where A.Material ='Picture' and A.BU='" & P_BU & "' AND B.Exam_Type = 'OP'"
        Else
            StrSql = "Select distinct [group] from Exercise_Detail As A With(nolock) inner join OP_ExamSubject As B With(nolock) On A.[Group] = B.Title where A.Material ='Picture' and A.BU='" & P_BU & "' AND B.Exam_Type = 'IDL'"
        End If
        '--superchai add 20231101 (E)
    ...
    Private Sub cmdAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdAdd.Click
        ...
        sFilename = cmbQuestionGroup.Text.Trim & "_" & cmbQuestionNO.Text.Trim & "_" & Date.Now().ToString("yyyyMMddHHmmss")        'superchai add 20231112

        '--superchai add 20231021 (B)
        If TStation.ToUpper() = "TRAINING" Then
            StrSql = "Exec InsertPictureExercise_TEST '" & Trim(cmbQuestionNO.Text.Trim) & "',N'" & Trim(cmbQuestionGroup.Text.Trim) & "',N'" & Trim(txtExercise.Text.Trim) & "'," &
                                    "N'" & Trim(TxtA.Text) & "',N'" & Trim(TxtB.Text) & "',N'" & Trim(txtC.Text) & "',N'" & Trim(txtD.Text) & "'," &
                                    "N'" & Trim(txtCorrectAnswer.Text.Trim) & "',N'" & Trim(cmbMaterial.Text.Trim) & "', N'" & Trim(sFilename) & "','" & P_BU & "',N'" & Trim(inputString) & "',N'" & Trim(txtComment.Text) & "', 'OP'"
        Else
            StrSql = "Exec InsertPictureExercise_TEST '" & Trim(cmbQuestionNO.Text.Trim) & "',N'" & Trim(cmbQuestionGroup.Text.Trim) & "',N'" & Trim(txtExercise.Text.Trim) & "'," &
                                    "N'" & Trim(TxtA.Text) & "',N'" & Trim(TxtB.Text) & "',N'" & Trim(txtC.Text) & "',N'" & Trim(txtD.Text) & "'," &
                                    "N'" & Trim(txtCorrectAnswer.Text.Trim) & "',N'" & Trim(cmbMaterial.Text.Trim) & "', N'" & Trim(sFilename) & "','" & P_BU & "',N'" & Trim(inputString) & "',N'" & Trim(txtComment.Text) & "', 'IDL'"
        End If
        '--superchai add 20231021 (E)

        'StrSql = "Exec InsertPictureExercise_TEST '" & Trim(cmbQuestionNO.Text.Trim) & "',N'" & Trim(cmbQuestionGroup.Text.Trim) & "',N'" & Trim(txtExercise.Text.Trim) & "'," &
        '                            "N'" & Trim(TxtA.Text) & "',N'" & Trim(TxtB.Text) & "',N'" & Trim(txtC.Text) & "',N'" & Trim(txtD.Text) & "'," &
        '                            "N'" & Trim(txtCorrectAnswer.Text.Trim) & "',N'" & Trim(cmbMaterial.Text.Trim) & "', N'" & Trim(sFilename) & "','" & P_BU & "','" & Trim(inputString) & "',N'" & Trim(txtComment.Text) & "'"
        Conn.Execute(StrSql)
        ClearPhotoExamData()
        MsgBox("数据保存PASS！")
    End Sub
    ...
    Private Sub cmbQuestionNO_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbQuestionNO.Click
        ...
        StrSql = "Select Question_NO from Exercise_Detail With(nolock) where [Group] =N'" & Trim(cmbQuestionGroup.Text) & "' and  Material ='Picture' and BU='" & P_BU & "' AND [Version] = (Select TOP 1 [Version] From Exercise_Detail With(nolock) Where [Group] =N'" & Trim(cmbQuestionGroup.Text) & "' and  Material ='Picture' and BU='" & P_BU & "' Order by CONVERT(int, [Version]) DESC) order by Question_NO"  'superchai add 20231101
        rs = Conn.Execute(StrSql)
        While rs.EOF = False
