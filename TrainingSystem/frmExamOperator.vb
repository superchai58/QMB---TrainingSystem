Public Class frmExamOperator
    ...
    Private Sub frmExamOperator_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'StrSql = "select distinct ID from OP_ExamSchedule where BU ='" & P_BU & "' "

        '-----------superchai add 20231028 (B)-------------
        If TStation.ToUpper() = "TRAINING" Then
            StrSql = "Select distinct ID from OP_ExamSchedule As A inner join OP_ExamSubject As B With(nolock) On A.Exam_No = B.Exam_NO where A.BU ='" & P_BU & "' AND B.Exam_Type = 'OP'"
        Else
            StrSql = "Select distinct ID from OP_ExamSchedule As A inner join OP_ExamSubject As B With(nolock) On A.Exam_No = B.Exam_NO where A.BU ='" & P_BU & "' AND B.Exam_Type = 'IDL'"
        End If
    ...
    Private Sub RefreshData(ByVal strExamOperator As String, ByVal ExamScheduleID As String)
        ...
        StrSql = "select * from OP_ExamOperator  where ExamScheduleID = '" + ExamScheduleID + "' and  BU=" & sq(P_BU) & "  order by EmployeeID"      'superchai modify 20231030
        rst = Conn.Execute(StrSql)
        If rst.EOF = False Then
    ...
    Private Sub cboExamScheduleID_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboExamScheduleID.SelectedIndexChanged
        ...
        'StrSql = "OP_GetExamScheduleInfo_Test @ExamScheduleID=" & sq(cboExamScheduleID.Text.Trim) & ",@BU=" & sq(P_BU) & ",@Type=" & sq(TStation.ToUpper())     'superchai add 20230920
        '-----------superchai add 20230824 (B)-------------
        If TStation.ToUpper() = "TRAINING" Then
            StrSql = "Exec OP_GetExamScheduleInfo_Test " & sq(cboExamScheduleID.Text.Trim) & ", '', " & sq(P_BU) & ", 'OP'"       'superchai add 20230920
        Else
            StrSql = "Exec OP_GetExamScheduleInfo_Test " & sq(cboExamScheduleID.Text.Trim) & ", '', " & sq(P_BU) & ", 'IDL'"       'superchai add 20230920
        End If
        '-----------superchai add 20230824 (E)-------------
        rst = Conn.Execute(StrSql)
        If rst.EOF = False Then
            While Not rst.EOF
                txtTitle.Items.Add(rst.Fields("Title").Value.ToString())
                rst.MoveNext()
            End While
            'txtTitle1.Text = rst.Fields("Title").Value
        End If
    End Sub
